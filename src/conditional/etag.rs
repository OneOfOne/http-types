...
/// use http_types_rs::conditional::ETag;
///
...
impl ETag {
	...
	pub(crate) fn from_str(s: &str) -> crate::Result<Self> {
		...
		let s = match s.strip_prefix('"').and_then(|s| s.strip_suffix('"')) {
			Some(s) => s.to_owned(),
			None => return Err(Error::from_str(StatusCode::BadRequest, "Invalid ETag header")),
		};

		if !s.bytes().all(|c| c == 0x21 || (0x23..=0x7E).contains(&c) || c >= 0x80) {
			return Err(Error::from_str(StatusCode::BadRequest, "Invalid ETag header"));
		}
