use futures::{io, prelude::*, AsyncBufRead, AsyncRead};
...
impl Request {
	...
	pub fn host(&self) -> Option<&str> {
		self.forwarded_header_part("host")
			.or_else(|| self.header("X-Forwarded-Host").and_then(|h| h.as_str().split(',').next()))
	...
	fn forwarded_for(&self) -> Option<&str> {
		self.forwarded_header_part("for")
			.or_else(|| self.header("X-Forwarded-For").and_then(|header| header.as_str().split(',').next()))
	}
	...
	///
	/// ```
	/// # fn main() -> Result<(), http_types_rs::Error> {
	/// #
	/// use http_types_rs::{Request, Response, StatusCode};
	/// let mut req = Request::get("https://example.com");
	...
	/// # fn main() -> Result<(), http_types_rs::Error> {
	/// #
	/// use http_types_rs::{Method, Request, Response, StatusCode, Url};
	/// let mut req = Request::get("https://example.com");
	...
	///
	/// ```
	/// use http_types_rs::{Method, Request};
	///
	/// let mut req = Request::get("https://example.com");
	/// req.set_body("Hello, Nori!");
	/// ```
	...
	/// # fn main() -> http_types_rs::Result<()> { async_std::task::block_on(async {
	/// #
	/// use http_types_rs::{Body, Method, Request};
	///
	/// let mut req = Request::get("https://example.com");
	/// req.set_body("Hello, Nori!");
	...
	/// # fn main() -> http_types_rs::Result<()> { async_std::task::block_on(async {
	/// #
	/// use http_types_rs::{Body, Request};
	///
	/// let mut req = Request::get("https://example.com");
	/// req.set_body("Hello, Nori!");
	...
	/// ```
	/// # fn main() -> http_types_rs::Result<()> { async_std::task::block_on(async {
	/// use http_types_rs::convert::{Deserialize, Serialize};
	/// use http_types_rs::{Body, Request};
	///
	...
	/// # Ok(()) }
	/// ```
	pub fn insert_header(&mut self, name: impl Into<HeaderName>, values: impl ToHeaderValues) -> crate::Result<Option<HeaderValues>> {
	...
	/// # Ok(()) }
	/// ```
	pub fn append_header(&mut self, name: impl Into<HeaderName>, values: impl ToHeaderValues) -> crate::Result<()> {
	...
	///
	/// ```
	/// use http_types_rs::{Request, Version};
	///
	/// # fn main() -> Result<(), http_types_rs::Error> {
	/// #
	/// let mut req = Request::get("https://example.com");
	...
	pub fn recv_trailers(&mut self) -> trailers::Receiver {
		let receiver = self.trailers_receiver.take().expect("Trailers receiver can only be constructed once");
	...
	///
	/// ```
	/// use http_types_rs::convert::Deserialize;
	/// use http_types_rs::Request;
	...
	pub fn query<'de, T: serde::de::Deserialize<'de>>(&'de self) -> crate::Result<T> {
		// Default to an empty query string if no query parameter has beserde_crateen specified.
		// This allows successful deserialisation of structs where all fielserde_crateds are optional
	...
	///
	/// ```
	/// use http_types_rs::convert::Serialize;
	/// use http_types_rs::{Method, Request};
	/// use std::collections::HashMap;
	///
	/// #[derive(Serialize)]
	...
	pub fn set_query(&mut self, query: &impl Serialize) -> crate::Result<()> {
		let query = serde_qs::to_string(query).map_err(|e| crate::Error::from_str(crate::StatusCode::BadRequest, format!("{}", e)))?;
...
mod tests {
	...
	mod host {
		...
		fn when_only_one_x_forwarded_hosts_exist() {
			let mut request = build_test_request();
			request.insert_header("x-forwarded-host", "expected.host").unwrap();
			assert_eq!(request.host(), Some("expected.host"));
		}
	...
	mod remote {
		...
		fn when_forwarded_is_improperly_formatted() {
			let mut request = build_test_request();
			request.set_peer_addr(Some("127.0.0.1:8000".parse::<std::net::SocketAddr>().unwrap()));

			request.insert_header("Forwarded", "this is an improperly ;;; formatted header").unwrap();
		...
		fn when_x_forwarded_for_is_set() {
			let mut request = build_test_request();
			request.set_peer_addr(Some(std::path::PathBuf::from("/dev/random").to_str().unwrap()));
	...
	fn set_x_forwarded_for(request: &mut Request, client: &'static str) {
		request
			.insert_header("x-forwarded-for", format!("{},proxy.com,other-proxy.com", client))
			.unwrap();
	}

	fn set_x_forwarded_host(request: &mut Request, host: &'static str) {
		request
			.insert_header("x-forwarded-host", format!("{},proxy.com,other-proxy.com", host))
			.unwrap();
	}

	fn set_forwarded(request: &mut Request, client: &'static str) {
		request
			.insert_header("Forwarded", format!("by=something.com;for={};host=host.com;proto=http", client))
			.unwrap();
	}
}
