...
impl Method {
	...
	pub fn is_safe(&self) -> bool {
		matches!(
			self,
			Method::Get | Method::Head | Method::Options | Method::Pri | Method::PropFind | Method::Report | Method::Search | Method::Trace
		)
	}
}
...
mod test {
	...
	fn names() -> Result<(), crate::Error> {
		...
		let methods = method_names.iter().map(|s| s.parse::<Method>()).collect::<Result<HashSet<_>, _>>()?;
