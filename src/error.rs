...
impl Error {
    ...
    {
        Self {
            status: status.try_into().expect("Could not convert into a valid `StatusCode`"),
            error: error.into(),
    ...
    where
        ...
        S::Error: Debug,
    {
        self.status = status.try_into().expect("Could not convert into a valid `StatusCode`");
    }
    ...
    /// Converts anything which implements `Display` into an `http_types_rs::Error`.
    ///
    ...
    /// Converts anything which implements `Debug` into an `http_types_rs::Error`.
    ///
