...
pin_project_lite::pin_project! {
      ...
      ///
      /// ```
      /// use http_types_rs::{Body, Response, StatusCode};
...
impl Body {
    ...
    /// ```
    pub fn from_reader(reader: impl AsyncBufRead + Unpin + Send + Sync + 'static, length: Option<u64>) -> Self {
        Self {
    ...
    pub async fn into_string(mut self) -> crate::Result<String> {
        ...
        self.read_to_string(&mut result).await.status(StatusCode::UnprocessableEntity)?;
        Ok(result)
    }
    ...
    ///
    /// ```
    /// use http_types_rs::{Body, convert::json};
    ///
    ...
    /// # fn main() -> http_types_rs::Result<()> { async_std::task::block_on(async {
    /// use http_types_rs::Body;
    /// use http_types_rs::convert::{Serialize, Deserialize};
    ///
    ...
    /// ```
    #[cfg(all(feature = "fs", not(target_os = "unknown")))]
    pub async fn from_file_with_path(mut file: async_std::fs::File, path: &std::path::Path) -> io::Result<Self> {
        let len = file.metadata().await?.len();

        // Look at magic bytes first, look at extension second, fall back to
        // octet stream.
        let mime = peek_mime(&mut file).await?.or_else(|| guess_ext(path)).unwrap_or(mime::BYTE_STREAM);
...
mod test {
    ...
    async fn attempting_to_read_past_length() -> crate::Result<()> {
        for buf_len in 1..13 {
            ...
            assert_eq!(read_with_buffers_of_size(&mut body, buf_len).await?, "hello");
    ...
    async fn attempting_to_read_when_length_is_greater_than_content() -> crate::Result<()> {
        for buf_len in 1..13 {
            ...
            assert_eq!(read_with_buffers_of_size(&mut body, buf_len).await?, "hello world");
    ...
    async fn chain_mixed_reader_string() -> crate::Result<()> {
        for buf_len in 1..13 {
            let mut body = Body::from_reader(Cursor::new("hello "), Some(6)).chain(Body::from("world"));
            assert_eq!(body.len(), Some(11));
            assert_eq!(body.mime(), Some(&mime::BYTE_STREAM));
            assert_eq!(read_with_buffers_of_size(&mut body, buf_len).await?, "hello world");
            assert_eq!(body.bytes_read, 11);
    ...
    async fn chain_mixed_nolen_len() -> crate::Result<()> {
        for buf_len in 1..13 {
            let mut body = Body::from_reader(Cursor::new("hello "), None).chain(Body::from("world"));
    ...
    async fn chain_mixed_len_nolen() -> crate::Result<()> {
        for buf_len in 1..13 {
            let mut body = Body::from("hello ").chain(Body::from_reader(Cursor::new("world"), None));
            assert_eq!(body.len(), None);
            assert_eq!(body.mime(), Some(&mime::BYTE_STREAM));
            assert_eq!(read_with_buffers_of_size(&mut body, buf_len).await?, "hello world");
            assert_eq!(body.bytes_read, 11);
    ...
    async fn chain_short() -> crate::Result<()> {
        for buf_len in 1..26 {
            let mut body = Body::from_reader(Cursor::new("hello xyz"), Some(6)).chain(Body::from_reader(Cursor::new("world abc"), Some(5)));
            assert_eq!(body.len(), Some(11));
            assert_eq!(body.mime(), Some(&mime::BYTE_STREAM));
            assert_eq!(read_with_buffers_of_size(&mut body, buf_len).await?, "hello world");
            assert_eq!(body.bytes_read, 11);
    ...
    async fn chain_many() -> crate::Result<()> {
        for buf_len in 1..13 {
            let mut body = Body::from("hello").chain(Body::from(&b" "[..])).chain(Body::from("world"));
            assert_eq!(body.len(), Some(11));
            assert_eq!(body.mime(), Some(&mime::BYTE_STREAM));
            assert_eq!(read_with_buffers_of_size(&mut body, buf_len).await?, "hello world");
            assert_eq!(body.bytes_read, 11);
