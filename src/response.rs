...
impl Response {
	...
	where
		...
		S::Error: Debug,
	{
		let status = status.try_into().expect("Could not convert into a valid `StatusCode`");
	...
	/// #
	/// use http_types_rs::{Method, Response, StatusCode, Url};
	///
	...
	/// #
	/// use http_types_rs::{Body, Method, Response, StatusCode, Url};
	///
	/// let mut req = Response::new(StatusCode::Ok);
	...
	/// # fn main() -> Result<(), http_types_rs::Error> {
	/// #
	/// use http_types_rs::{Response, StatusCode, Version};
	///
	/// let mut res = Response::new(StatusCode::Ok);
	...
	pub fn send_upgrade(&mut self) -> upgrade::Sender {
		...
		let sender = self.upgrade_sender.take().expect("Upgrade sender can only be constructed once");
	...
	pub async fn recv_upgrade(&mut self) -> upgrade::Receiver {
		self.has_upgrade = true;
		let receiver = self.upgrade_receiver.take().expect("Upgrade receiver can only be constructed once");
