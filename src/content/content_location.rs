...
impl ContentLocation {
    ...
    where
        ...
        U::Error: std::fmt::Debug,
    {
        self.url = location.try_into().expect("Could not convert into valid URL")
    }
}
...
mod test {
    ...
    fn smoke() -> crate::Result<()> {
        ...
        assert_eq!(content_location.location(), &Url::parse("https://example.net/test.json")?);
        Ok(())
    }
    ...
    fn bad_request_on_parse_error() {
        let mut headers = Headers::new();
        headers.insert(CONTENT_LOCATION, "htt://<nori ate the tag. yum.>").unwrap();
