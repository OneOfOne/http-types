...
/// #
/// use http_types_rs::content::ContentType;
...
impl ContentType {
	...
	{
		Self {
			media_type: media_type.try_into().expect("could not convert into a valid Mime type"),
		}
	}
...
mod test {
	...
	fn smoke() -> crate::Result<()> {
		...
		assert_eq!(ct.header_value(), format!("{}", Mime::from_str("text/*")?).as_str());
		Ok(())
	}
	...
	fn bad_request_on_parse_error() {
		let mut headers = Headers::new();
		headers.insert(CONTENT_TYPE, "<nori ate the tag. yum.>").unwrap();
