...
mod test {
	...
	fn smoke() -> crate::Result<()> {
		...
		assert_eq!(referer.location(), &Url::parse("https://example.net/test.json")?);
		Ok(())
	}
	...
	fn bad_request_on_parse_error() {
		let mut headers = Headers::new();
		headers.insert(REFERER, "htt://<nori ate the tag. yum.>").unwrap();
		let err = Referer::from_headers(Url::parse("https://example.net").unwrap(), headers).unwrap_err();
	...
	fn fallback_works() -> crate::Result<()> {
		...
		let referer = Referer::from_headers(base_url, headers)?.unwrap();
		assert_eq!(referer.location(), &Url::parse("https://fallback.net/test.json")?);

		let mut headers = Headers::new();
		headers.insert(REFERER, "https://example.com/test.json").unwrap();

		let base_url = Url::parse("https://fallback.net/")?;
		let referer = Referer::from_headers(base_url, headers)?.unwrap();
		assert_eq!(referer.location(), &Url::parse("https://example.com/test.json")?);
		Ok(())
	}
}
