use http_types_rs::{bail, ensure, ensure_eq, Error, StatusCode};
use std::io;
...
fn anyhow_error_into_http_types_error() {
    let anyhow_error = anyhow::Error::new(std::io::Error::new(std::io::ErrorKind::Other, "irrelevant"));
...
fn normal_error_into_http_types_error() {
    let http_types_error: Error = std::io::Error::new(std::io::ErrorKind::Other, "irrelevant").into();
    assert_eq!(http_types_error.status(), StatusCode::InternalServerError);

    let http_types_error = Error::new(StatusCode::ImATeapot, std::io::Error::new(std::io::ErrorKind::Other, "irrelevant"));
    assert_eq!(http_types_error.status(), StatusCode::ImATeapot);
}
...
fn u16_into_status_code_in_http_types_error() {
    ...
    let http_types_error2 = Error::new(StatusCode::NotFound, io::Error::new(io::ErrorKind::Other, "Not Found"));
...
fn fail_test_u16_into_status_code_in_http_types_error_new() {
    let _http_types_error = Error::new(1000, io::Error::new(io::ErrorKind::Other, "Incorrect status code"));
}
