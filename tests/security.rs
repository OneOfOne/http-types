use http_types_rs::{security, Response, StatusCode};
...
fn security_test() {
    ...
    assert_eq!(
        ...
        "base-uri 'none'; default-src 'self' areweasyncyet.rs; object-src 'none'; script-src 'self' 'unsafe-inline'; upgrade-insecure-requests"
    );
}
